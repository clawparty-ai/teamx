...
impl<'a> Socket<'a> {
        ...
        T: Into<IpListenEndpoint>,
    {
        ...
        // [teamx patch] allow port 0 as a wildcard for transparent proxying:
        // a listening socket with port 0 accepts SYNs to any destination port.
        // (Upstream rejects port 0 as "unaddressable".)
    ...
    pub(crate) fn accepts(&self, _cx: &mut Context, ip_repr: &IpRepr, repr: &TcpRepr) -> bool {
        ...
        if let Some(tuple) = &self.tuple {
            ...
            // [teamx patch] port 0 = wildcard: accept SYNs to any destination
            // port (transparent proxy). Upstream requires an exact port.
