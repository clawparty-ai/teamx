...
| 阶段 | 内容 | 验收 |
|---|---|---|
| I0 | Rust PKI：CA 生成、server/member 证书签发、**强制 mTLS** serve（rustls）；证书默认 3650d | serve 启动即 mTLS；无明文降级；curl 双向认证通过 |
| I1 | `team invite` 签发 letter；`team import` 导入并 mTLS 连接 | 成员凭 letter 建立连接、身份=证书身份 |
| I2 | **approve 流程**：join pending → owner approve → active；吊销（revoke）检查、一次性使用、角色自动授予 | 证书泄露不能直接工作；revoke 后立即断连；join 自动获得角色 |
| I3 | 插件集成：mTLS transport + auto-execute 对接 | 成员加入自动进入工作模式、收任务自动执行 |
| I4 | 跨网络验证（局域网/公网） | 两台机器 mTLS 协作闭环 |
