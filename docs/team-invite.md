...
> 状态：**I0/I1 已实现**（PKI + 强制 mTLS serve + `team invite`/`team import` + 证书身份接入 RPC），I2（吊销检查 + approve 流程 + 插件集成）进行中
...
| I2 | **approve 流程**：join pending → owner approve → active；吊销（revoke）检查、一次性使用、角色自动授予 | 🔄 部分完成（invite-list/revoke 已实现；证书连接时吊销检查待补） |
| I3 | 插件集成：mTLS transport + auto-execute 对接 | 🔄 部分完成（runRpc mTLS + 工具已实现） |
| I4 | 跨网络验证（局域网/公网） | ⬜ 待做 |
